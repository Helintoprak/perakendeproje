    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
          {filteredEdu.map((edu) => (
            canManage && effectiveView === 'uploaded'
              ? <EducationCardAdmin key={edu.id} edu={edu as EducationAdmin} deleting={deleting === edu.id} onDelete={() => handleDelete(edu.id, edu.title)} />
...
function EducationCardAdmin({ edu, deleting, onDelete }: {
  edu: EducationAdmin; deleting: boolean; onDelete: () => void;
}) {
